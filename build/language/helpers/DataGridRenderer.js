...
var DataGridRenderer = {
  ...
  asp: function(dataGrid, headerNames, headerTypes, indent, newLine) {
    ...
    outputText =
      'Dim myArray(' + (j - 1) + ',' + (i - 1) + ')' + newLine + outputText
  ...
  html: function(dataGrid, headerNames, headerTypes, indent, newLine) {
    ...
    for (var j = 0; j < numColumns; j++) {
      outputText +=
        indent + indent + indent + '<th class="' + headerNames[j] + '-cell">'
    ...
    for (var i = 0; i < numRows; i++) {
      ...
      for (var j = 0; j < numColumns; j++) {
        outputText +=
          indent + indent + indent + '<td class="' + headerNames[j] + '-cell">'
        outputText += row[j]
    ...
    for (var i = 0; i < numRows; i++) {
      ...
      if (numColumns == 2) {
        ...
        for (var j = 1; j < numColumns; j++) {
          if (j > 1) outputText += ', '
          outputText +=
            '"' + headerNames[j] + '"' + ':' + _fmtVal(i, j, dataGrid)
        }
        outputText += '}'
      }
      if (i < numRows - 1) {
  ...
  xmlIllustrator: function(
    ...
    outputText +=
      indent + '<variableSet  varSetName="binding1" locked="none">' + newLine
    ...
    for (var i = 0; i < numColumns; i++) {
      outputText +=
        ...
        '<variable varName="' +
        headerNames[i] +
        '" trait="textcontent" category="&ns_flows;"></variable>' +
        newLine
    }
    outputText += indent + indent + '</variables>' + newLine
    outputText +=
